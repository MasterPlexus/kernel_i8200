...
#if defined(CONFIG_COND_N_POWER_USE)
	struct delayed_work power_check_work;
	struct wake_lock power_check_wake_lock;
	int power_check;
	int cond_n;
	int cond_n_irq;
	int cond_n_stat;
...
#if defined(CONFIG_COND_N_POWER_USE)
static void sx9500_irq_power_check_work(struct work_struct *work)
{
	struct sx9500_p *data = container_of((struct delayed_work *)work,
			struct sx9500_p, power_check_work);

	static int grip_power_val = 0;
	int cnt = 0, ret = 0;

	grip_power_val = data->cond_n_stat;

	if(grip_power_val == 1) {
		pr_info("[SX9500]: %s - grip sensor power off"
			"data->flagDataSkip = %d\n",
			__func__, (int)data->flagDataSkip);
		/* release event */
		input_report_rel(data->input, REL_MISC, 2);
		input_sync(data->input);
	} else if(grip_power_val == 0) {
		pr_info("[SX9500]: %s - grip sensor power off->on\n", __func__);

		for (cnt = 0; cnt < TOTAL_BOTTON_COUNT; cnt++)
			data->state[cnt] = IDLE;

		data->touchMode = INIT_TOUCH_MODE;
		ret = sx9500_i2c_write(data, SX9500_SOFTRESET_REG,
			SX9500_SOFTRESET);
		if(ret < 0)
			pr_err("[SX9500]: %s - failed soft reset\n", __func__);
		...
		ret = sx9500_i2c_write(data, SX9500_CPS_CTRL0_REG,
			0x20 | ENABLE_CSX);
		if(ret < 0)
			pr_err("[SX9500]: %s - failed enable csx\n", __func__);
		msleep(20);
		...
		pr_info("[SX9500]: %s - initialize register done\n", __func__);
	}
	data->power_check = 1;
}
#endif
...
#if defined(CONFIG_COND_N_POWER_USE)
static irqreturn_t sx9500_interrupt_power_check_thread(int irq, void *pdata)
{
	struct sx9500_p *data = pdata;

	pr_info("[SX9500]: %s - called\n", __func__);

	if(data->power_check == 1) {
		data->power_check = 0;
		data->cond_n_stat = gpio_get_value(data->cond_n);
		wake_lock_timeout(&data->power_check_wake_lock, 4 * HZ);
		schedule_delayed_work(&data->power_check_work,
			msecs_to_jiffies(100));
	} else
		pr_info("[SX9500]: %s - denied\n", __func__);
	return IRQ_HANDLED;
}
#endif
...
static int sx9500_setup_pin(struct sx9500_p *data, struct i2c_client *client)
{
	int ret;
	...
	pr_err("[SX9500]: %s called data->gpioNirq=(%d) \n",
			__func__, data->gpioNirq);
	...
	if (ret < 0) {
		pr_err("[SX9500]: %s - failed to set gpio %d input (%d)\n",
			__func__, data->gpioNirq, ret);
	...
	if(pdata->gpio_en_rf_touch) {
		data->en_rf_touch = pdata->gpio_en_rf_touch;

		ret = gpio_request(data->en_rf_touch, "SX9500_en_rf_touch");
		if (ret < 0) {
			pr_err("[SX9500]: %s - gpio %d request failed (%d)\n",
				__func__, data->en_rf_touch, ret);
			return ret;
		}

		ret = gpio_direction_output(data->en_rf_touch, 0);

		if (ret < 0) {
			pr_err("[SX9500]: %s - failed to set gpio %d output"
				"(%d)\n", __func__, data->en_rf_touch, ret);
			gpio_free(data->en_rf_touch);
		}

	} else
		pr_info("[SX9500]: %s - data->en_rf_touch = NULL\n", __func__);

	if(pdata->gpio_cond_n) {
		data->cond_n = pdata->gpio_cond_n;

		ret = gpio_request(data->cond_n, "SX9500_gpio_cond_n");
		if (ret < 0) {
			pr_err("[SX9500]: %s - gpio %d request failed (%d)\n",
				__func__, data->cond_n, ret);
			return ret;
		}

		ret = gpio_direction_input(data->cond_n);
		if (ret < 0) {
			pr_err("[SX9500]: %s - failed to set gpio %d input "
				"(%d)\n", __func__, data->cond_n, ret);
			gpio_free(data->cond_n);
		}

		data->cond_n_irq = gpio_to_irq(data->cond_n);

		ret = request_threaded_irq(data->cond_n_irq, NULL,
			sx9500_interrupt_power_check_thread,
			IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING |
			IRQF_ONESHOT,
			"sx9500_power_irq", data);
		if (ret < 0) {
			pr_err("[SX9500]: %s - failed to request threaded"
				" cond n irq \n", __func__);
			return ret;
		}

		disable_irq(data->cond_n_irq);
	} else
		pr_info("[SX9500]: %s - data->cond_n= NULL\n", __func__);
#endif
	return 0;
}
		...
		const struct i2c_device_id *id)
{
	...
	ret = sx9500_setup_pin(data, client);
	if (ret) {
...
#if defined(CONFIG_COND_N_POWER_USE)
	wake_lock_init(&data->power_check_wake_lock,
		WAKE_LOCK_SUSPEND, "grip_power_wake_lock");
	data->power_check = 1;
#endif
...
#if defined(CONFIG_COND_N_POWER_USE)
	INIT_DELAYED_WORK(&data->power_check_work, sx9500_irq_power_check_work);

	/* for cond_n irq */
	enable_irq(data->cond_n_irq);
	enable_irq_wake(data->cond_n_irq);
#endif
...
#if defined(CONFIG_COND_N_POWER_USE)
	cancel_delayed_work_sync(&data->power_check_work);
	free_irq(data->cond_n_irq, data);
	gpio_free(data->cond_n);
	gpio_free(data->en_rf_touch);
#endif
	free_irq(data->irq, data);
