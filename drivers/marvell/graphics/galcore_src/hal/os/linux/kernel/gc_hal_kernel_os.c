    ...
    )
{
    ...
    mdl->kaddr      = vaddr;
#if ALLDYNAMIC
    trace_printk("%d@Paddr0x%x,F:%s,D:%d\n",(gctUINT32)bytes,mdl->dmaHandle,Function,Line);
#endif
#if gcdNONPAGED_MEMORY_CACHEABLE
    addr = vaddr;
#else
        ...
        while (PageCount-- > 0)
        {
...
#endif
            {
                ...
                else if(mdl->bPmem)
                {
                    ...
                    gckOS_GetPageSize(Os, &pagesize);
                    gcmkONERROR(
                        gckMMU_SetPage(Os->device->kernels[Core]->mmu,
                             (mdl->dmaHandle + offset*pagesize),
                             table));
                }
                else
                {
